 ...
 * 
 * The constructor doesn't take any parameters to create a LeapYearCalculator.
 * 
 * The first method isLeapYear(year) checks if given year is a Leap Year and displays the answer.
 * 
 * The second method displayCalendar(year) displays a calendar for given year, without days of the week,
 ...
 * 
 * The third method displayCalendarDays(year) displays an enhanced calendar, considering days of the week.
 ...
 */
 ...
 public class LeapYearCalculator { 
		...
		int month = 0;// number of the month. 1 for January.
		int year = 2016; //change for desired year
		boolean leapYear = false;//will be changed if it's a leap year

		//Constructor for the leap year calculator
		public LeapYearCalculator(){
		}

		//Method that verifies and prints out whether it's a leap year or not.
		public void isLeapYear(int yr){
			year = yr;
			if (year <=0){
		...
		//method that takes a year and displays its calendar, without days of the week.	
		public void displayCalendar(int yr){
			...
			if (year <=0){
				...
				// loop to create and display each month.
		...
		//Method that takes a year and displays its calendar with correct days of the week.
		...
		year = yr;
			...
			// Algorithm to determine the day of the week of 1st of January of given year, based on Gauss' formula (https://en.wikipedia.org/wiki/Determination_of_the_day_of_the_week):
			int a = 6*((year-1)%400);
			int b = 4*((year-1)%100);
			int c = 5*((year-1)%4);
			int startDay = (1+a+b+c)%7;
			//End of algorithm, startDay is set to 1 if Monday.
			...
			int monthStart = 0;//will be negative if the month doesn't start on Sunday, in order to print empty spaces.
				...
				//loop to generate and display each month.
				for (month = 1; month <= 12; month ++){
					...
					if (month == 1){
						monthStart = (-startDay+1)%7; // setting the amount of empty spaces to display. Week starts on Sunday.
						System.out.println("        January "+year);
